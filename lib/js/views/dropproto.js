...
DropIt.DropProtoView = Backbone.View.extend({
    ...
    selectChanged: function(event) {
        ...
        if ( this.subview === undefined )
            this.subview = new DropIt.DropProtoSubView({ model: this.model, type: opt });
        else 
            this.subview.render( { model: this.model, type: opt } );
    ...
    addDrop: function( event ) {
        ...
        // Serialize fieldset to save
        var item = this.$el.find('fieldset').serializeObject();
...
DropIt.DropProtoSubView = Backbone.View.extend({
    ...
    events: {
        'click .button-add-query-arg': 'cloneQueryArg'
    },
    ...
    render: function( args ) {
        this.template = _.template( jQuery( '#'  + args.type + '_create_drop_template' ).html() );
        this.$el.html( this.template( _.extend( this.model.toJSON(), args ) ) );
        return this;
    },

    cloneQueryArg: function(e) {
        e.preventDefault();
        var select = this.$('.drop-query-param-wrapper').filter(":first").clone();
        this.$(e.target).before(select);
    }
});