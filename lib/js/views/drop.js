...
DropIt.DropView = Backbone.View.extend({
    ...
    initialize: function ( args ) {
         ...
         console.log( args );
         // We need to use globally defined jQuery
         // because this.$ is scoped within the view element
        this.template = _.template( jQuery( '#common_drop_template' ).html() );
    ...
    editDrop: function( e ) {
        e.preventDefault();
        this.$el.find('.di-drop').removeClass('di-drop-collapsed').addClass('di-drop-extended');
/*        this.$('.widget-inside').hide();
    ...
    saveDrop: function( e ) {
        e.preventDefault();
        this.$el.find('.di-drop').addClass('di-drop-collapsed').addClass('di-drop-extended');
